...
public class AlathraExtrasData {
    ...
    private File getFile(String strID) {
        ...
        File userFile = new File("plugins" + File.separator + "AlathraExtras" + File.separator + "userdata" + File.separator + strID + ".yml");
        if (!userFile.exists()) {
            try {
                userFile.createNewFile();
            } catch (IOException e) {
                Bukkit.getLogger().warning("[AlathraExtras] Encountered error when creating user file for " + strID);
            }
        }
        return userFile;
    }
